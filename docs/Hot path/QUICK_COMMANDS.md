...
### 🚀 One-Click Start: 3 Workers Song Song
```powershell
# Khởi chạy 3 terminals song song:
...
# Custom settings
.\scripts\start_parallel_scrapers.ps1 -MaxKols 10 -MaxVideos 30 -Interval 600
```

### Chạy từng worker riêng (4 terminals)
```powershell
# Terminal 1: Discovery scraper (tìm username mới)
.\.venv\Scripts\python.exe -m ingestion.sources.kol_scraper daemon --discovery-only --interval 300

# Terminal 2: Video Stats Worker (lấy profile + video stats)
.\.venv\Scripts\python.exe -m ingestion.consumers.video_stats_worker --max-videos 20

# Terminal 3: Comment Extractor (lấy comments)
.\.venv\Scripts\python.exe -m ingestion.consumers.comment_extractor --max-comments 50

# Terminal 4: Product Extractor (lấy products từ TikTok Shop)
.\.venv\Scripts\python.exe -m ingestion.consumers.product_extractor --max-videos 20
```
...
Prerequisites:
- `kol-redis` container reachable as `redis:6379` on the Docker network
- `redis` Python package installed in the Spark driver container
  (install inside `kol-spark-master` or ship wheel via `--py-files`)

Run (in PowerShell):
```powershell
# Install redis client inside spark master container (one-time)
docker exec kol-spark-master /bin/bash -lc "pip install redis"

# Submit trending job (hot path)
docker exec kol-spark-master /opt/spark/bin/spark-submit `
    --master spark://spark-master:7077 `
...
### Bước 1: Chạy Discovery + 3 Workers Song Song
```powershell
# Terminal 1: Discovery
.\.venv\Scripts\python.exe -m ingestion.sources.kol_scraper daemon --discovery-only

# Terminal 2: Video Stats (lấy profile + videos)
.\.venv\Scripts\python.exe -m ingestion.consumers.video_stats_worker --max-videos 20

# Terminal 3: Comments (optional)
.\.venv\Scripts\python.exe -m ingestion.consumers.comment_extractor --max-comments 50
...
### Bước 2: Chạy Spark ETL (load Kafka → MinIO)
```powershell
docker exec kol-spark-master /opt/spark/bin/spark-submit `
    --master spark://spark-master:7077 `
    --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.0,org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.4.2,org.apache.hadoop:hadoop-aws:3.3.4 `
...
### Bước 3: Chạy Product Tracker (track sold_count changes)
```powershell
