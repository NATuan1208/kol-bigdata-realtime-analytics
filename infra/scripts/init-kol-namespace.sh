...
docker exec sme-minio mc alias set local http://localhost:9000 minioadmin minioadmin 2>/dev/null || true
...
docker exec -i sme-postgres psql -U admin -d postgres <<EOF
-- Create KOL MLflow database (separate from SME databases)
CREATE DATABASE kol_mlflow;
GRANT ALL PRIVILEGES ON DATABASE kol_mlflow TO admin;

-- Create KOL metadata database (optional, for future use)
CREATE DATABASE kol_metadata;
GRANT ALL PRIVILEGES ON DATABASE kol_metadata TO admin;
